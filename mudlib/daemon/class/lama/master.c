...
void create()
{
	set_name("龍若法王", ({ "master lama", "master", "lama" }) );
...
string ask_for_join()
{
	...
	if( (string)me->query("class")=="bonze" )
		return "阿彌陀佛！你我同是出家人，何故跟老衲開這等玩笑？\n";
	if( (string)me->query("gender") != "男性" )
		return "阿彌陀佛！女施主，這裏是寺廟，請□到尼庵去剃度吧。\n";
...
void attempt_apprentice(object ob)
{
	if( query("apprentice_available") ) {
		...
		command("say 老納今天已經收了十個弟子，不想再收徒了。");
	}
}
...
void do_recruit(object ob)
{
	if( (string)ob->query("gender") != "男性" )
		command("say 阿彌陀佛，女施主不要跟老納開玩笑。");
	else if( (string)ob->query("class") != "bonze" )
		command("say 阿彌陀佛，施主願入佛門，請先到小寺剃度出家。");
	else {
...
{
	if( (string)me->query("class")=="bonze" )
		command("say 阿彌陀佛！出家人戒逞強惡鬥！老衲不敢違反清規。");
	else
		command("say 施主既然身負絕藝，老衲佩服便是，也不必較量了。");
